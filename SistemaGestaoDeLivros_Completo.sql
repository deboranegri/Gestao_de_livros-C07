...
INSERT IGNORE INTO Perfil_Usuario (userName, preferenciasDeLeitura) VALUES
...
INSERT IGNORE INTO Livro (titulo, preco, estoque) VALUES
...
INSERT IGNORE INTO Cliente (nome, email, dataNascimento, fkPerfilUsuario) VALUES
...
INSERT IGNORE INTO Pedido (dataPedido, fkCliente) VALUES
...
INSERT IGNORE INTO Item_Pedido (fkPedido, fkLivro, quantidade, precoUnitario) VALUES
(1, 2, 1, 60.50),
(2, 4, 2, 35.00),
(3, 1, 1, 150.00),
(4, 5, 1, 180.00),
...
INSERT IGNORE INTO Pagamento (dataPagamento, formaPagamento, valorTotal, fkPedido) VALUES
...
-- 6. VERIFICAÇÃO — SELECT * FROM todas as tabelas

SELECT 'TABELA: Perfil_Usuario' AS Info; SELECT * FROM Perfil_Usuario;
SELECT 'TABELA: Livro'          AS Info; SELECT * FROM Livro;
SELECT 'TABELA: Cliente'        AS Info; SELECT * FROM Cliente;
SELECT 'TABELA: Pedido'         AS Info; SELECT * FROM Pedido;
SELECT 'TABELA: Item_Pedido'    AS Info; SELECT * FROM Item_Pedido;
SELECT 'TABELA: Pagamento'      AS Info; SELECT * FROM Pagamento;

-- JOINs de verificação
SELECT 'JOIN 1 — Clientes e Perfis' AS Info;
SELECT c.nome, p.userName, p.preferenciasDeLeitura
FROM Cliente c JOIN Perfil_Usuario p ON c.fkPerfilUsuario = p.idPerfilUsuario;

SELECT 'JOIN 2 — Pedidos e Clientes' AS Info;
SELECT p.idPedido, p.dataPedido, c.nome
FROM Pedido p JOIN Cliente c ON p.fkCliente = c.idCliente;

SELECT 'JOIN 3 — Relatório de Vendas (VIEW)' AS Info;
